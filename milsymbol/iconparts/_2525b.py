from .iconparts_functions import defaultProperties


def _2525b(ms, iconParts, metadata, colors, STD2525, monoColor, alternateMedal):
    ...
    # user "iconColor" from colors dict.
    # JS: const iconColor = colors.iconColor[affiliation];
    # In Python colors is likely a dict of dicts or objects.
    # Need to handle get properly.

    if isinstance(colors.get("iconColor"), dict):
        iconColor = colors["iconColor"].get(affiliation)
    else:
        iconColor = getattr(colors.get("iconColor"), affiliation, None)
