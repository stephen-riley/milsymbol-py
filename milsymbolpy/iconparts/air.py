...
def air(ms, iconParts, metadata, colors, STD2525, monoColor, alternateMedal):
    frame = metadata.get("frame")
    affiliation = metadata.get("affiliation", "Friend")
    numberSIDC = metadata.get("numberSIDC")

    # Helper to clean up color access
    def get_color(prop, aff):
        c = colors.get(prop)
        if isinstance(c, dict):
            return c.get(aff)
        return getattr(c, aff, None)

    iconColor = get_color("iconColor", affiliation)
    iconFillColor = get_color("iconFillColor", affiliation)
    black = get_color("black", affiliation)
    ...
    icn["AR.I.CIVILIAN ROTARY WING"] = {
        "type": "path",
        "fill": iconFillColor if (STD2525 or numberSIDC) else False,
        "stroke": black,
        "d": "M60,85 l40,15 40,-15 0,30 -40,-15 -40,15 z",
    }
    ...
    icn["AR.I.VSTOL"] = text("L" if (STD2525 and not numberSIDC) else "V")
    icn["AR.I.AIRBORNE COMMAND POST"] = (
        text("D") if (STD2525 and not numberSIDC) else text("ACP")
    )
    icn["AR.I.AIRBORNE EARLY WARNING"] = (
        text("W") if (STD2525 and not numberSIDC) else text("AEW")
    )
    icn["AR.I.ANTISURFACE WARFARE"] = (
        text("N") if (STD2525 and not numberSIDC) else text("ASUW")
    )
    icn["AR.I.ANTISUBMARINE WARFARE"] = (
        text("S") if (STD2525 and not numberSIDC) else text("ASW")
    )
    icn["AR.I.COMMUNICATIONS"] = (
        text("Y") if (STD2525 and not numberSIDC) else text("COM")
    )
    icn["AR.I.COMBAT SEARCH AND RESCUE"] = text("CSAR")
    icn["AR.I.ELECTRONIC SUPPORT MEASURES"] = text("Z") if STD2525 else text("ESM")
    icn["AR.I.GOVERNMENT"] = text("GOV")
    icn["AR.I.MINE COUNTERMEASURES"] = text("MCM")
    icn["AR.I.PERSONNEL RECOVERY"] = text("H") if STD2525 else text("PRO")
    ...
    icn["AIR.M1.VSTOL"] = textm1("L" if (STD2525 and not numberSIDC) else "V")
    icn["AIR.M1.PASSENGER"] = textm1("PX")
    icn["AIR.M1.ULTRA LIGHT"] = textm1("UL")
    icn["AIR.M1.AIRBORNE COMMAND POST"] = (
        textm1("D") if (STD2525 and not numberSIDC) else textm1("ACP")
    )
    icn["AIR.M1.ANTISURFACE WARFARE"] = (
        textm1("N") if (STD2525 and not numberSIDC) else textm1("ASUW")
    )
    icn["AIR.M1.AIRBORNE EARLY WARNING"] = (
        textm1("W") if (STD2525 and not numberSIDC) else textm1("AEW")
    )
    ...
    icn["AIR.M1.PHOTOGRAPHIC"] = (
        textm1("X") if (STD2525 and not numberSIDC) else textm1("PH")
    )
    icn["AIR.M1.PERSONNEL RECOVERY"] = textm1("PR")
    icn["AIR.M1.ANTISUBMARINE WARFARE"] = (
        textm1("S") if (STD2525 and not numberSIDC) else textm1("ASW")
    )
    icn["AIR.M1.COMMUNICATIONS"] = (
        textm1("Y") if (STD2525 and not numberSIDC) else textm1("COM")
    )
    icn["AIR.M1.ELECTRONIC SURVEILLANCE MEASURES"] = (
        textm1("Z") if STD2525 else textm1("ESM")
    )
    ...
    icn["AIR.M1.COMBAT SEARCH AND RESCUE"] = (
        textm1("H") if (STD2525 and not numberSIDC) else textm1("CSAR")
    )
    ...
    # 2525D
    icn["AIR.M1.ELECTRONIC SUPPORT (ES)"] = textm1("ES") if STD2525 else textm1("ESM")
    ...
    if STD2525 or numberSIDC:
        icn["AIR.MISSILE.ICON"]["fill"] = (
            colors.get("fillColor", {}).get("Unknown")
            if frame
            else colors.get("iconFillColor", {}).get("Unknown")
        )
